...
class Neuralynx:
    ...
    def read_header(fid):
        ...
        pos = fid.tell()
        fid.seek(0)
        raw_hdr = fid.read(Neuralynx.HEADER_LENGTH).strip(b'\0')
        fid.seek(pos)
    ...
    def parse_header(raw_hdr):
        ...
        hdr[u'TimeOpened_dt'] = Neuralynx.parse_neuralynx_time_string(hdr_lines[2])
        hdr[u'TimeClosed'] = hdr_lines[3][3:]
        hdr[u'TimeClosed_dt'] = Neuralynx.parse_neuralynx_time_string(hdr_lines[3])
    ...
    def estimate_record_count(file_path, record_dtype):
        ...
        file_size -= Neuralynx.HEADER_LENGTH
    ...
    def load_ncs(file_path, load_time=True, rescale_data=True, signal_scaling=MICROVOLT_SCALING):
        ...
        with open(file_path, 'rb') as fid:
            raw_header = Neuralynx.read_header(fid)
            records = Neuralynx.read_records(fid, Neuralynx.NCS_RECORD)

        header = Neuralynx.parse_header(raw_header)
        Neuralynx.check_ncs_records(records)
    ...
    def load_nev(file_path):
        ...
        with open(file_path, 'rb') as fid:
            raw_header = Neuralynx.read_header(fid)
            records = Neuralynx.read_records(fid, Neuralynx.NEV_RECORD)
    ...
    def read_nev(file):
        nev = Neuralynx.load_nev(file)

        ts = pd.DataFrame(list(nev['events']['TimeStamp']), columns=[0])
        ts[0] = ts[0].div(1000000)

        ttl = pd.DataFrame(list(nev['events']['ttl']), columns=[2])
        ...
        for row in ttl[2]:
            if row < 0:
                hexa.append(int2hex(row, 32))
            else:
                hexa.append(int2hex(row, 32))
        ttl[1] = hexa

        ttl[1] = ttl[1].map(lambda x: str(x)[2:])
        ttl[1] = ttl[1].str.upper()
        del ttl[2]

        data = pd.concat([ts, ttl], axis=1)
